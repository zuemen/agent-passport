...
import { explorerAddr, explorerTx, isLocal, localMonadRules, short } from "./chain";
import {
  approve,
  createPasskey,
  loadPasskey,
  ...
  return (
    <div className="passkey">
      ...
      <p className="sub">
        ...
        action is one biometric approval, verified on-chain by {!isLocal ? "Monad's P-256 precompile" : localMonadRules ? "the P-256 precompile of the local chain's Monad rules" : "OpenZeppelin's Solidity P-256 check (this anvil has no precompile; Foundry 1.8+ runs Monad's rules)"}; a relayer pays the gas.
      </p>
      {!pk ? (
